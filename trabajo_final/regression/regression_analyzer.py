"""
trabajo_final/regression/regression_analyzer.py
...
Partición: 80/20, random_state=42
Métricas: MAE, RMSE, R²
...
class RegressionAnalyzer:
    """
    Entrena y evalúa KNN, RandomForest y DecisionTree sobre
    base_simulada_streaming.csv.
    ...
    def __init__(
        self,
        data_path: str = "trabajo_final/data/base_simulada_streaming.csv",
        ...
        if not os.path.exists(data_path):
            raise FileNotFoundError(
                ...
                "Ejecute primero el módulo Montecarlo para generar base_simulada_streaming.csv."
            )
    ...
    def load_data(self) -> tuple:
        """
        Carga base_simulada_streaming.csv con pandas.
        Verifica que tiene las columnas requeridas.
        Retorna (X, y) donde y = columna 'riesgo_qos'.
        """
    ...
    def generate_scatter_plot(self) -> None:
        """
        Genera docs/comparativo_difuso_vs_prediccion_streaming.png con el mejor modelo.
        """
        if not self._trained_models:
            raise RuntimeError(
                "Los modelos no han sido entrenados. "
        ...
        ax.scatter(
            self._y_test, y_pred,
            alpha=0.5, edgecolors="steelblue",
        ...
        ax.set_title(
            f"Predicciones vs. Valores Difusos — {nombre_modelo} (Streaming)",
            fontsize=13
        )
        ax.set_xlabel("Valores Difusos Reales (riesgo_qos)", fontsize=11)
        ax.set_ylabel("Predicciones del Modelo", fontsize=11)
        ax.legend()
        ax.grid(True, linestyle="--", alpha=0.4)

        output_path = os.path.join(
            self.docs_dir, "comparativo_difuso_vs_prediccion_streaming.png"
        )
        fig.tight_layout()
    ...
    def generate_comparative_report(self, metrics: dict) -> None:
        """
        Genera docs/regression_comparativa_streaming.md con tabla de métricas.
        """
        ...
        if all(m["r2"] < self.MIN_R2_THRESHOLD for m in metrics.values()):
            lines += [
                ...
                "",
                "Todos los modelos evaluados presentan R² inferior al umbral mínimo "
                f"de {self.MIN_R2_THRESHOLD:.2f}. Se recomienda revisar la calidad "
                "de los datos o explorar modelos más complejos.",
                "",
            ]
        ...
        lines += [
            ...
            "- **Random Forest (100 árboles):** Ensemble de 100 árboles de decisión "
            "con `random_state=42`. Robusto ante sobreajuste.",
            "- **Decision Tree:** Árbol de decisión individual con `random_state=42`. "
            "Alta interpretabilidad.",
            "- **SVR (RBF):** Support Vector Regression con kernel RBF. "
            "Usa escalado estándar y parámetros C=100, gamma='scale', epsilon=0.1. "
        ...
        output_path = os.path.join(self.docs_dir, "regression_comparativa_streaming.md")
    ...
    def generate_importance_report(self) -> None:
        """
        Genera docs/regression_importancia_streaming.md con tablas de
        ...
        lines = [
            ...
            "variable de entrada al poder predictivo del modelo. Los valores suman 1.0.",
            "",
        ]
        ...
        lines += [
            ...
            "Una importancia alta indica alta capacidad discriminativa para predecir "
            "el nivel de riesgo QoS en la plataforma de streaming.",
            "",
        ...
        output_path = os.path.join(self.docs_dir, "regression_importancia_streaming.md")
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("\n".join(lines))
    ...
    def generate_comparative_analysis(self, metrics: dict, correlation: float) -> None:
        """
        Genera docs/analisis_comparativo_streaming.md con tabla de métricas,
        correlación de Pearson, importancias y sección de Trazabilidad.
        """
        if not self._trained_models:
            raise RuntimeError(
                "Los modelos no han sido entrenados. "
        ...
        # Mapa de trazabilidad desde consenso Delphi
        trazabilidad_map = {}
        ...
        lines = [
            ...
            "modelos estadísticos de regresión supervisada para la plataforma de streaming.",
            "",
            "---",
            "",
        ...
        lines += [
            ...
            "capturan patrones similares en los datos de la plataforma de streaming.",
            "",
            "---",
            "",
        ...
        lines += [
            ...
            "La consistencia entre ambos modelos refuerza la validez de los "
            "factores priorizados en el proceso Delphi.",
            "",
            "---",
            "",
        ...
        lines += [
            ...
            f"al predecir el índice de riesgo QoS generado por el sistema difuso Mamdani. "
            ...
            "estadísticos, validando la coherencia metodológica del sistema.",
            "",
        ]

        output_path = os.path.join(self.docs_dir, "analisis_comparativo_streaming.md")
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("\n".join(lines))
